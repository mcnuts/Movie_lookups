...
my $file_type = scraper {
       ...
       my $type = $file_type->scrape( URI->new("$kat") );
		...
		$ok=0;
		if($k=~m/(.avi|.mkv|.mp4)/){ 
			if($k!~m/(CD01.avi|CD1.avi|CD2.avi|CD02.avi|CD01.mp4|CD02.mp4|.r01|.rar)/){
				$ok=1;
			}
		}
	}
...
if($res->is_success){
	...
	my $file_type = scraper {
       ...
       my $type = $file_type->scrape( URI->new("$t_hound") );
		...
		$ok=0;
		if($t=~m/(.avi|.mkv|.mp4)/){
			if($t!~m/(CD01.avi|CD1.avi|CD2.avi|CD02.avi|CD01.mp4|CD02.mp4|.r01|.rar)/){
				$ok=1;
			}
		}
	}
...
for (@$tlist){
	...
	my $t_files=$rt->file_list($hash);
		...
		foreach(@files){
			$file=$_;
				if($file=~m/.r00|.r01|.rar|CD01|CD02|cd01|cd02|.zip/){
